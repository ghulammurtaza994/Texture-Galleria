...
function sendOrderNotification(order) {
  ...
  if (!TWILIO_ACCOUNT_SID || !TWILIO_AUTH_TOKEN || !TWILIO_FROM_NUMBER) {
    ...
    console.log(`[SMS] Twilio credentials are not configured. Opening WhatsApp chat: ${whatsappUrl}`);
  ...
  return new Promise((resolve) => {
    const req = https.request(options, (res) => {
      ...
      res.on('end', () => {
        if (res.statusCode >= 200 && res.statusCode < 300) {
          ...
          console.error(`[SMS] Failed to send notification. Status: ${res.statusCode}. ${payload}`);
    ...
    req.on('error', (err) => {
      console.error('[SMS] Error while sending notification:', err.message);
